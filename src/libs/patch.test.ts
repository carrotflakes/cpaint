...
describe('applyPatch', () => {
  ...
  it('should move a value within an array', () => {
    ...
    const result = applyPatch(obj, patch);
    expect(result).toEqual({ a: [2, 3, 1, 4] });
  });

  it('should throw an error for an invalid path', () => {
    const obj = { a: 1 };
    const patch: Patch = { op: 'add', path: [], value: 2 };
...
describe('reversePatch', () => {
  ...
  it('should reverse a move operation within an array', () => {
    const obj = { a: [1, 2, 3, 4] };
    const patch: Patch = { op: 'move', from: ['a', 2], to: ['a', 0] };
    const reversed = reversePatch(obj, patch);
    expect(reversed).toEqual({ op: 'move', from: ['a', 0], to: ['a', 2] });
  });
});
